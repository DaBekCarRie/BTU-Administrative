...
describe("isBackdated — ตัวตัดสินว่าต้องเล่นเหตุการณ์ใหม่ทั้งเส้นหรือไม่", () => {
  const call = (occurredAt: string): DomainEvent => ({
    type: "โทรตาม",
    occurredAt,
    payload: { outcome: "ไม่รับสาย" },
  });

  it("เหตุการณ์ที่เกิดก่อนของล่าสุด ต้องเล่นใหม่", () => {
    expect(
      isBackdated("2026-09-10T03:00:00.000Z", call("2026-09-01T03:00:00.000Z")),
    ).toBe(true);
  });

  it("เหตุการณ์ที่เกิดหลังของล่าสุด พับทับได้เลย", () => {
    expect(
      isBackdated("2026-09-01T03:00:00.000Z", call("2026-09-10T03:00:00.000Z")),
    ).toBe(false);
  });

  it("เวลาเท่ากันเป๊ะ ไม่ถือว่าย้อนหลัง — ตัวที่เพิ่งบันทึกมาทีหลังอยู่แล้ว", () => {
    expect(
      isBackdated("2026-09-10T03:00:00.000Z", call("2026-09-10T03:00:00.000Z")),
    ).toBe(false);
  });

  it("เทียบด้วยเวลาจริง ไม่ใช่เทียบสตริง — รูปแบบ offset ต่างกันก็ต้องถูก", () => {
    // เวลาเดียวกัน เขียนคนละรูป: +07:00 ของไทย กับ Z
    expect(
      isBackdated("2026-09-10T10:00:00+07:00", call("2026-09-10T03:00:00.000Z")),
    ).toBe(false);
    // เทียบแบบสตริงจะได้ผลผิด เพราะ "2026-09-10T02:59" < "2026-09-10T10:00"
    expect(
      isBackdated("2026-09-10T10:00:00+07:00", call("2026-09-10T02:59:00.000Z")),
    ).toBe(true);
  });
});
